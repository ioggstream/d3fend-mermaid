import { E as Be, F as Ze, R as Xe, G as qe, H as Mn, I as $t, J as Dn, K as ye, L as ke, c as xt, s as _n, g as Sn, x as Un, y as Yn, b as Fn, a as Ln, M as nt, z as En, e as An, u as In, l as qt, j as Pt, k as Wn, f as On } from "./mermaid-50853933.js";
import { b as Hn, t as Ue, c as Nn, a as Vn, l as zn } from "./linear-47a64fe4.js";
...
function Ge(t, e) {
  ...
  function w(x) {
    var q = r ?? (e.ticks ? e.ticks.apply(e, n) : e.domain()), g = i ?? (e.tickFormat ? e.tickFormat.apply(e, n) : Zn), L = Math.max(s, 0) + y, O = e.range(), W = +O[0] + S, B = +O[O.length - 1] + S, Z = (e.bandwidth ? jn : Gn)(e.copy(), S), Q = x.selection ? x.selection() : x, b = Q.selectAll(".domain").data([null]), A = Q.selectAll(".tick").data(q, e).order(), T = A.exit(), Y = A.enter().append("g").attr("class", "tick"), M = A.select("line"), v = A.select("text");
    b = b.merge(b.enter().insert("path", ".tick").attr("class", "domain").attr("stroke", "currentColor")), A = A.merge(Y), M = M.merge(Y.append("line").attr("stroke", "currentColor").attr(C + "2", k * s)), v = v.merge(Y.append("text").attr("fill", "currentColor").attr(C, k * L).attr("dy", t === Bt ? "0em" : t === ue ? "0.71em" : "0.32em")), x !== Q && (b = b.transition(x), A = A.transition(x), M = M.transition(x), v = v.transition(x), T = T.transition(x).attr("opacity", Ye).attr("transform", function(o) {
      ...
      return F(Z(o) + S);
    }), M.attr(C + "2", k * s), v.attr(C, k * L).text(g), Q.filter(Qn).attr("fill", "none").attr("font-size", 10).attr("font-family", "sans-serif").attr("text-anchor", t === te ? "start" : t === Rt ? "end" : "middle"), Q.each(function() {
      this.__axis = Z;
    });
  }
...
const on = bt(0), jt = bt(1), ur = bt(2), fr = bt(3), Dt = bt(4), hr = bt(5), dr = bt(6);
on.range;
jt.range;
ur.range;
fr.range;
...
function gr(t, e, n, r, i, s) {
  ...
  function S(k, C, F) {
    ...
    if (x === 0)
      return Mt.every(Math.max(Ue(k, C, F), 1));
...
function pr(t) {
  ...
  function b(p, E) {
    return function(D) {
      ...
      for (D instanceof Date || (D = /* @__PURE__ */ new Date(+D)); ++R < j; )
        p.charCodeAt(R) === 37 && (l.push(p.slice(N, R)), (et = Le[J = p.charAt(++R)]) != null ? J = p.charAt(++R) : et = J === "e" ? " " : "0", (Ut = E[J]) && (J = Ut(D, et)), l.push(J), N = R + 1);
      return l.push(p.slice(N, R)), l.join("");
    };
  }
  function A(p, E) {
    return function(D) {
      var l = Yt(1900, void 0, 1), R = T(l, p, D += "", 0), N, j;
      if (R != D.length)
        return null;
      if ("Q" in l)
        return new Date(l.Q);
  ...
  function T(p, E, D, l) {
    for (var R = 0, N = E.length, j = D.length, J, et; R < N; ) {
      if (l >= j)
        return -1;
      if (J = E.charCodeAt(R++), J === 37) {
        if (J = E.charAt(R++), et = Q[J in Le ? E.charAt(R++) : J], !et || (l = et(p, D, l)) < 0)
          return -1;
      } else if (J != D.charCodeAt(l++))
        return -1;
    }
    return l;
  }
  function Y(p, E, D) {
    var l = k.exec(E.slice(D));
    return l ? (p.p = C.get(l[0].toLowerCase()), D + l[0].length) : -1;
  }
  function M(p, E, D) {
    var l = x.exec(E.slice(D));
    return l ? (p.w = q.get(l[0].toLowerCase()), D + l[0].length) : -1;
  }
  function v(p, E, D) {
    var l = F.exec(E.slice(D));
    return l ? (p.w = w.get(l[0].toLowerCase()), D + l[0].length) : -1;
  }
  function o(p, E, D) {
    var l = O.exec(E.slice(D));
    return l ? (p.m = W.get(l[0].toLowerCase()), D + l[0].length) : -1;
  }
  function d(p, E, D) {
    var l = g.exec(E.slice(D));
...
function ui(t, e) {
  return t = fn(t), P(Dt.count(pt(t), t) + (pt(t).getUTCDay() === 4), e, 2);
}
function fi(t) {
  return t.getUTCDay();
}
  ...
  var t = function(v, o, d, m) {
    ...
    parse: function(o) {
      ...
      for (var D, l, R, N, j = {}, J, et, Ut, zt; ; ) {
        if (l = m[m.length - 1], this.defaultActions[l] ? R = this.defaultActions[l] : ((D === null || typeof D > "u") && (D = E()), R = X[l] && X[l][D]), typeof R > "u" || !R.length || !R[0]) {
          var Kt = "";
          zt = [];
          for (J in X[l])
...
` + I.showPosition() + `
Expecting ` + zt.join(", ") + ", got '" + (this.terminals_[D] || D) + "'" : Kt = "Parse error on line " + (h + 1) + ": Unexpected " + (D == H ? "end of input" : "'" + (this.terminals_[D] || D) + "'"), this.parseError(Kt, {
            text: I.match,
        ...
        if (R[0] instanceof Array && R.length > 1)
          throw new Error("Parse Error: multiple actions possible at state: " + l + ", token: " + D);
        switch (R[0]) {
          case 1:
            m.push(D), _.push(I.yytext), c.push(I.yylloc), m.push(R[1]), D = null, U = I.yyleng, f = I.yytext, h = I.yylineno, it = I.yylloc;
            break;
          case 2:
...
(function(t, e) {
  (function(n, r) {
    ...
    function q(g) {
      var L, O;
      L = g, O = y && y.formats;
      for (var W = (g = L.replace(/(\[[^\]]+])|(LTS?|l{1,4}|L{1,4})/g, function(Y, M, v) {
        var o = v && v.toUpperCase();
        return M || O[v] || n[v] || O[o].replace(/(\[[^\]]+])|(MMMM|MM|DD|dddd)/g, function(d, m, u) {
          return m || u.slice(1);
        });
      ...
      return function(Y) {
        for (var M = {}, v = 0, o = 0; v < B; v += 1) {
          var d = W[v];
          if (typeof d == "string")
            o += d.length;
          else {
            var m = d.regex, u = d.parser, _ = Y.slice(o), c = m.exec(_)[0];
            u.call(M, c), Y = Y.replace(c, "");
          }
        }
        return function(X) {
          var f = X.afternoon;
    ...
    return function(g, L, O) {
      ...
      W.parse = function(Z) {
        ...
        var T = A[1];
        if (typeof T == "string") {
          var Y = A[2] === !0, M = A[3] === !0, v = Y || M, o = A[2];
          M && (o = A[2]), y = this.$locale(), !Y && o && (y = O.Ls[o]), this.$d = function(_, c, X) {
            try {
              if (["x", "X"].indexOf(c) > -1)
                return new Date((c === "X" ? 1e3 : 1) * _);
              var f = q(c)(_), h = f.year, U = f.month, G = f.day, H = f.hours, V = f.minutes, I = f.seconds, z = f.milliseconds, st = f.zone, it = /* @__PURE__ */ new Date(), p = G || (h || U ? 1 : it.getDate()), E = h || it.getFullYear(), D = 0;
              h && !U || (D = U > 0 ? U - 1 : it.getMonth());
              var l = H || 0, R = V || 0, N = I || 0, j = z || 0;
              return st ? new Date(Date.UTC(E, D, p, l, R, N, j + 60 * st.offset * 1e3)) : X ? new Date(Date.UTC(E, D, p, l, R, N, j)) : new Date(E, D, p, l, R, N, j);
            } catch {
...
nt.extend(Di);
nt.extend(Si);
let at = "", be = "", ve, xe = "", Ht = [], Nt = [], we = {}, Ce = [], Jt = [], _t = "", Me = "";
const yn = ["active", "done", "crit", "milestone"];
let De = [], Vt = !1, _e = !1, Se = "sunday", de = 0;
const Ui = function() {
  Ce = [], Jt = [], _t = "", De = [], Zt = 0, ge = void 0, Xt = void 0, K = [], at = "", be = "", Me = "", ve = void 0, xe = "", Ht = [], Nt = [], Vt = !1, _e = !1, de = 0, we = {}, En(), Se = "sunday";
}, Yi = function(t) {
  be = t;
}, Fi = function() {
  return be;
...
const hs = function(t, e, n, r) {
  ...
  if (r.db.getDisplayMode() === "compact" || i.displayMode === "compact") {
    ...
    for (const M of Object.keys(T)) {
      const v = fs(T[M], Y) + 1;
      Y += v, x += v * (i.barHeight + i.barGap), w[M] = v;
    }
  } else {
  ...
  function L(T, Y) {
    const M = T.startTime, v = Y.startTime;
    let o = 0;
    return M > v ? o = 1 : M < v && (o = -1), o;
  }
  ...
  function O(T, Y, M) {
    ...
    B(
      ...
      r.db.getExcludes(),
      r.db.getIncludes()
    ), Z(m, d, Y, M), W(T, o, d, m, v, u, Y), Q(o, d), b(m, d, Y, M);
  }
  function W(T, Y, M, v, o, d, m) {
    ...
    if (c.append("rect").attr("id", function(h) {
      ...
      return U = h.order, (g(h.startTime) + v + 0.5 * (g(h.endTime) - g(h.startTime))).toString() + "px " + (U * Y + M + 0.5 * o).toString() + "px";
      ...
      return U = h.order, U * Y + i.barHeight / 2 + (i.fontSize / 2 - 2) + M;
  ...
  function B(T, Y, M, v, o, d, m, u) {
    ...
    q.append("g").selectAll("rect").data(f).enter().append("rect").attr("id", function(H) {
      ...
      return (g(H.start) + M + 0.5 * (g(H.end) - g(H.start))).toString() + "px " + (V * T + 0.5 * o).toString() + "px";
  ...
  function Q(T, Y) {
    ...
    q.append("g").selectAll("text").data(v).enter().append(function(o) {
      ...
      if (d > 0)
        for (let m = 0; m < d; m++)
          return M += v[d - 1][1], o[1] * T / 2 + M * T + Y;
      else
        return o[1] * T / 2 + Y;
  ...
  function A(T) {
    ...
    for (let v = 0, o = T.length; v < o; ++v)
      Object.prototype.hasOwnProperty.call(Y, T[v]) || (Y[T[v]] = !0, M.push(T[v]));
    return M;
  }
}, ds = {
  setConf: us,
